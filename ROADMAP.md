...
| 版本 | 主题 | 内容（简） |
|---|---|---|
| **v0.2.0** | 可选 online enrichment | 独立 `enrich()` 模块，调用方拿到离线结果后可选并入在线字段；可关闭；**不改 `query_ip` 内部** |
| **v0.3.0** | SQLite 写入串行化 | 按 `docs/SQLITE_CONCURRENCY_TODO.md`：busy_timeout → 写锁/writer 队列 → 事务原子化 → 错误分类 → 压测 |
| **v0.4.0** | GUI 优化 | 审计后再改：响应性、错误提示、enrichment 展示（可关闭） |
